...
class CheckModel(object):
    def __init__(self):
        self.vocab_path = os.path.join(config.data_path, 'vocab_11.pkl')
        ...
        self.model_path = os.path.join(config.model_path, r'TextCNN_witsky_11.ckpt')
        self.stop_word_map = {
            'A:': 0,
            'B:': 0,
            "你好": 0,
    ...
    def decode_text(self, content_list):
        contents = []
        amout_len = len(content_list)
        count_num = 0
        for line in content_list:
            lin = line.strip()
            print('{}/{}'.format(amout_len, count_num))
            count_num += 1
            if not lin:
                continue
            label = 0
    ...
    def get_label(self, content_list: list):
        ...
        test_iter = DatasetIterater(one_code_list, 1, torch.device('cuda:1' if torch.cuda.is_available() else 'cpu'))

        embedding = 'embedding_SougouNews.npz'
        config_model = TextCNN.Config(dataset='data_witsky', embedding=embedding)
        model = TextCNN.Model(config_model).to('cuda:1')
    ...
    def get_label_pd(self):
        data_pd = pd.read_excel(os.path.join(config.data_path, 'mqi_rcs_report_字段调整_step3.xlsx'),
        ...
        print(len(items_list), len(result_items_list))
        data_pd['item_sample'] = items_list
        data_pd['result_items_7'] = result_items_list
        data_pd.to_excel(os.path.join(config.data_path, 'mqi_rcs_report_result.xlsx'),
                         index=False)
