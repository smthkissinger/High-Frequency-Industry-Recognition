...
vocab_path = os.path.join(config.data_path, 'vocab.pkl')
train_path = os.path.join(config.data_path, 'train.txt')
dev_path = os.path.join(config.data_path, 'dev.txt')
...
class CheckModel(object):
    def __init__(self):
        self.vocab_path = os.path.join(config.data_path, 'vocab.pkl')
        ...
        self.model_path = os.path.join(config.model_path, r'TextCNN_witsky_8.ckpt')
        self.stop_word_map = {
            'A:': 0,
            'B:': 0,
            "你好": 0,
